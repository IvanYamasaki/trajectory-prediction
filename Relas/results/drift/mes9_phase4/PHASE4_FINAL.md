    ...
    (configuração: `delta=1e-7, mw=600, cd=200, xw=2000`). É o único detetor que
    combina cobertura total dos cinco anos pós-pandemia (2021–2025) com
    taxa de falsos alarmes baixa e controlada.

    | Métrica | Valor | Notas |
    |---------|-------|-------|
    | FAR\_2019\_per1k | 0.0417 | IC Wilson 95%: [0.0114, 0.1519]/1k |
    | year\_coverage | 5/5 | Deteta em todos os anos 2021–2025 |
    | SNR\_smooth (a=1) | 1.6000 | Substituto finito e estável do SNR\_op |
    | detection\_efficiency | 0.9200 | 92.0% dos alarmes são pós-2019 |
    | n\_post (alarmes 2021–2025) | 23 | Distribuídos por 5 anos |

    O valor FAR=0.0417/1k corresponde a
    2 alarmes no período de linha de base (2019),
    ...
    - ADWIN\_S2S dispara o primeiro alarme em índice **~67,585**
      (após acumular buffer suficiente com mistura 2019+2021).
    - **Δ₁ = 18,310 trajetórias** entre os primeiros alarmes do mesmo evento.
    - Par mais próximo: ADWIN@80,911, PH@81,125,
      **Δ\_min = 214 trajetórias**.

    Para a porta AND disparar, é necessário W ≥ Δ\_min = 214 trajetórias.
    O sweep de W confirma que W=500 (≥ 214) produz exactamente
    ...
    | ADWIN\_S2S standalone | 11 | 1 | 5 | 4 | 2 | 5/5 |
...
| 20000 | 5.0 | 4 | 0.000 | 2 |
    ...
    **Interpretação:** Para W <= 2000 (até ~0.5 meses de dados), o AND confirma
    ...
    | Métrica | Valor |
    |---------|-------|
    | FAR\_2019\_per1k | 0.1458 |
    | year\_coverage | 5/5 |
    | SNR\_smooth (a=1) | 1.7250 |
    | detection\_efficiency | 0.9067 |
    | n\_post | 68 |

    O canal OR cobre os mesmos 5/5 anos que o ADWIN standalone, com maior
    sensibilidade (n\_post=68 vs 23)
    à custa de FAR mais alto (0.146 vs
    0.042/1k). Recomendado apenas como
...
| --- | --- | --- | --- | --- |
| ADWIN_S2S | 2.30 | 1.60 | 5 | Não |
| ADWIN_Kalman | 1.80 | 1.53 | 5 | Não |
| OR(ADWIN_S2S, ADWIN_Kalman) | 1.94 | 1.73 | 5 | Não |
...
| --- | --- | --- | --- |
| 0.5 | 1 | ADWIN_S2S | 1.8800 |
| 0.5 | 2 | ADWIN_Kalman | 1.6545 |
| 0.5 | 3 | OR(ADWIN_S2S, ADWIN_Kalman) | 1.8267 |
| 1.0 | 1 | ADWIN_S2S | 1.6000 |
| 1.0 | 2 | ADWIN_Kalman | 1.5333 |
| 1.0 | 3 | OR(ADWIN_S2S, ADWIN_Kalman) | 1.7250 |
| 2.0 | 1 | ADWIN_S2S | 1.2500 |
| 2.0 | 2 | ADWIN_Kalman | 1.3429 |
| 2.0 | 3 | OR(ADWIN_S2S, ADWIN_Kalman) | 1.5556 |
    ...
    **Resposta (confirmada — limitação estrutural dupla):**
    ...
    (Δ\_min=214 trajs, Factor 1 da Sec. 2) tornaria o AND inoperante
    com W razoável.
    ...
    **Resposta:** Com as métricas corrigidas, o trade-off é:
    ...
    |-------|---------|--------|----------------|-----------------|
    | Alto rigor | ADWIN\_S2S standalone | 0.0417 | 5/5 | Re-treino automático |
    | Alta cobertura | OR/SUSPECTED | 0.1458 | 5/5 | Monitorização reforçada |
    ...
    **Resposta: Não. O AND temporal NÃO adiciona valor sobre o ADWIN\_S2S standalone.**
    ...
    1. Desalinhamento temporal de 18,310 trajetórias no primeiro alarme 2021
       (Δ\_min=214 trajetórias no par mais próximo).
